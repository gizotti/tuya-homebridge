...
// Home app's color wheel sends near-zero saturation for its "white" spot instead of a
// ColorTemperature command. Saturation at/below this % is treated as picking white.
const WHITE_SATURATION_THRESHOLD = 5;
...
class LightAccessory extends BaseAccessory {
  ...
  getSendParam(name, value) {
    ...
    switch (name) {
      ...
      case Characteristic.ColorTemperature:
        ...
        value = temperature;
        var workModeCmd = this._getWorkModeCommand('light_white', 'white');
        if (workModeCmd) {
          commands = [{ code, value }, workModeCmd];
        }
        break;
      ...
      case Characteristic.Hue:
        var bright;
        var saturation;
        var isWhiteSelection = this.getCachedState(Characteristic.Saturation) <= WHITE_SATURATION_THRESHOLD;
        var workModeCmd = isWhiteSelection
          ? this._getWorkModeCommand('light_white', 'white')
          : this._getWorkModeCommand('colour', 'colour');
        if (isWhiteSelection && workModeCmd) {
          // near-zero saturation means "white" was picked on the color wheel; colour_data is meaningless in white mode
          commands = [workModeCmd];
        } else {
  ...
  //device's DP schema (not live status) tells us whether it supports work_mode and which enum string it uses
  _getWorkModeCommand(preferred, fallback) {
    const workModeFunc = this.functionArr.find(f => f.code === 'work_mode');
    if (!workModeFunc) return null;
    let value = fallback;
    if (workModeFunc.values) {
      const range = JSON.parse(workModeFunc.values).range || [];
      if (range.includes(preferred)) value = preferred;
    }
    return { code: workModeFunc.code, value };
  }
