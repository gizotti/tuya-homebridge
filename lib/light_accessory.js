...
class LightAccessory extends BaseAccessory {
    ...
    switch (name) {
      ...
      case Characteristic.ColorTemperature:
        ...
        if (this.workMode && this.workMode.value !== 'white' && this.workMode.value !== 'light_white') {
          commands = [
            { code, value },
            { code: this.workMode.code, value: this._getWorkModeValue('light_white', 'white') }
          ];
        }
        break;
      ...
      case Characteristic.Hue:
        ...
        if (this.workMode && this.workMode.value !== 'colour') {
          commands = [
            { code, value },
            { code: this.workMode.code, value: 'colour' }
          ];
        }
        break;
      default:
  ...
  //resolve the enum value this device actually uses for a work_mode (e.g. 'white' vs 'light_white')
  _getWorkModeValue(preferred, fallback) {
    const workModeFunc = this.functionArr.find(f => f.code === 'work_mode');
    if (workModeFunc && workModeFunc.values) {
      const range = JSON.parse(workModeFunc.values).range || [];
      if (range.includes(preferred)) return preferred;
    }
    return fallback;
  }
